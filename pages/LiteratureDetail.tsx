...
const LiteratureDetail: React.FC = () => {
  ...
  const handleCommentSubmit = async (newComment: Omit<Comment, 'id' | 'timestamp'>) => {
    ...
    setWritings(prevWritings => prevWritings.map(w => 
        w.id === id 
        ? { ...w, comments: [...w.comments, { ...newComment, id: `c${Date.now()}`, timestamp: new Date().toISOString() }] } 
        : w
    ));
  };
  ...
  return (
    <PageWrapper>
      <article className="max-w-4xl mx-auto">
        ...
        {isNovel && <h2 className="text-center text-2xl text-white font-serif mb-8">{episodes[currentEpisodeIndex]?.title}</h2>}
